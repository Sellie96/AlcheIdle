...
export class WoodcuttingGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async create(@MessageBody() woodcutting: WoodcuttingDto) {
    ...
    const message: Woodcutting[] = this.woodCuttingService.addToWoodcuttingActive({username: woodcutting.username, message:woodcutting.message, time: new Date().toISOString().split("T")[1].split(".")[0]});
